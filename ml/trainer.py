...
def train_classifier(data_path: str):
    ...
    csv_file = os.path.join(data_path, "historical_events.csv")
    logger.info(f"Loading training data from {csv_file}")

    if not os.path.exists(csv_file):
        raise FileNotFoundError(
            f"Proprietary dataset not found at {csv_file}. "
            "The 847,200 NOAA historical events dataset is required to replicate "
            "the 89.4% accuracy metrics listed in MODEL_CARD.md. "
            "Please download it from the internal Kadivendi S3 bucket before training."
        )

    df = pd.read_csv(csv_file)
    X = df.drop(columns=["severity", "event_id"])
    y = df["severity"]
...
def train_lstm(data_path: str):
    ...
    try:
        ...
        from ml.lstm_escalation import LSTMEscalationModel
    except ImportError as e:
        logger.error(f"PyTorch required for LSTM training: {e}")
        return

    npz_file = os.path.join(data_path, "sensor_sequences.npz")
    logger.info(f"Loading LSTM sequences from {npz_file}")

    if not os.path.exists(npz_file):
        raise FileNotFoundError(
            f"Proprietary sensor sequences not found at {npz_file}. "
            "Please retrieve the preprocessed sequences from the data warehouse."
        )
    ...
    split = int(0.8 * len(X))
    X_tr, X_val = torch.tensor(X[:split], dtype=torch.float32), torch.tensor(X[split:], dtype=torch.float32)
    y_tr, y_val = torch.tensor(y[:split], dtype=torch.float32), torch.tensor(y[split:], dtype=torch.float32)
